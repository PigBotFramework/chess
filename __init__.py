...
class chess(PBF):
    ...
    def jing_pair(self):
        ...
        if self.data.args[-1] != self.data.args[0]:
            ...
            return self.client.msg().raw('先手：[CQ:at,qq={0}]\n请发送“井字棋下 X坐标 Y坐标”来下棋'.format(ob.get('turn')))
    ...
    def jing_go(self):
        ...
        zuobiFlag = True if self.data.args[2] != self.data.args[-1] and int(self.data.args[-1]) == ob.get('zuobi') else False
        if zuobiFlag:
    ...
    def make(self):
        ...
        frame.draw_text((115, bianchang*100+5, 300, bianchang*100+100), str(self.data.se.get('sender').get('nickname'))+'（绿方）')
    ...
    def join(self):
        ...
        frame.draw_text((bianchang*100-300, bianchang*100+5, bianchang*100-110, bianchang*100+100), str(self.data.se.get('sender').get('nickname'))+'（红方）')
